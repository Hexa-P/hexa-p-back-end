...
app.post('/api/user_profile', async(req, res) => {
  try {
    const data = await client.query(`
    INSERT INTO user_profile (month_param, city_api_id, fav_url, owner_id)
    VALUES ($1, $2, $3, $4)
    RETURNING *`
    , [
...
app.get('/temps', async(req, res) => {
  try {
    ...
    res.status(500).json({ error: e.message, headers: res.headers });
  }
});
...
app.get('/articles', async(req, res) => {
  try {
    const URL = 'https://newsapi.org/v2/everything?apiKey=0ae236b94d5944ab85612e8ee145b53c&q=climate';
    const response = await request
      .get(URL);
