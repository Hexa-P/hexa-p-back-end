...
app.delete('/api/user_profile/:id', async (req, res) => {
  try {
    ...
    const userProfileId = req.userId;

    const data = await client.query(`
    DELETE from user_profile 
    WHERE user_profile.owner_id = $1 
    AND user_profile.id = $2
...
app.get('/temps', async (req, res) => {
  try {
    ...
    const newResponse = mungeTemps(response.body);
...
app.post('/many_temps', async (req, res) => {
  try {

    const responses = [];

    for(const id of req.body.city_ids) {
      const URL = `https://app.climate.azavea.com/api/climate-data/${id}/historical/indicator/Average_High_Temperature/?years=1950:2005&time_aggregation=monthly`;
      ...
      responses.push({
        id,
        city: response.body.city.properties.name,
        latitude: response.body.city.geometry.coordinates[1],
        longitude: response.body.city.geometry.coordinates[0],
...
app.get('/articles', async (req, res) => {
  try {
    const URL = `https://newsapi.org/v2/everything?apiKey=${process.env.NEWS_KEY}&q=climate`;
    const response = await request
      .get(URL);
